...
int predictor_size = (TagSize + 30 + 1) * BtbSize; // Tag size + Target pc size - 2bits redundent + valid bit
    ...
    if(IsGlobalHist){
        ...
        predictor_size += (HistorySize * BtbSize);
	...
	int temp = (std::pow(2,HistorySize+1)) * (BtbSize);
